...
# Custom CSS (keeping blue accents to match theme)
st.markdown("""
<style>
    .main-header {
        font-size: 2.5rem;
...
# Initialize Engine (only if customer data is available)
engine = None
if df_customer is not None:
    engine = RecommendationEngine(df_forecast)

# Define groups for navbar
group_pages = {
    "Dashboards": ["📊 Executive Dashboard", "🏠 Action Dashboard"] if df_customer is not None else ["📊 Executive Dashboard"],
    "Forecasting": ["📈 Sales Forecasting (12 Months)", "🏷️ Brand-Level Forecasting", "🔮 Forecast Analysis & Insights"],
    "Customer Actions": ["👥 Customers & Leads", "📦 Inventory & Pricing", "🎯 Marketing Campaigns", "🤖 AI Sales Assistant"] if df_customer is not None else [],
    "Customer Analytics": ["👥 Customer Segmentation", "🚨 Churn Prediction", "💰 Customer Lifetime Value", "🔍 Customer Search"] if df_customer is not None else [],
    "Service & Sales": ["🔧 Service Optimization", "📊 Sales Insights"] if df_customer is not None else []
}

# Filter out empty groups
active_groups = {k: v for k, v in group_pages.items() if v}

# Horizontal Navbar for groups
selected_group = option_menu(
    menu_title=None,
    options=list(active_groups.keys()),
    icons=["bar-chart", "graph-up-arrow", "person-lines-fill", "clipboard-data", "wrench"],
    orientation="horizontal",
    styles={
        "container": {"background-color": "white", "padding": "0!important"},
...
# Second horizontal navbar for sub-pages
page = None
if selected_group in active_groups:
    group_options = active_groups[selected_group]
    group_icons = {
        "📊 Executive Dashboard": "bar-chart-fill",
        "🏠 Action Dashboard": "house-door-fill",
        "📈 Sales Forecasting (12 Months)": "graph-up",
        "🏷️ Brand-Level Forecasting": "tags-fill",
        "🔮 Forecast Analysis & Insights": "lightbulb-fill",
        "👥 Customers & Leads": "people-fill",
        "📦 Inventory & Pricing": "box-seam",
        "🎯 Marketing Campaigns": "bullseye",
        "🤖 AI Sales Assistant": "robot",
        "👥 Customer Segmentation": "diagram-3-fill",
        "🚨 Churn Prediction": "exclamation-triangle-fill",
        "💰 Customer Lifetime Value": "currency-dollar",
        "🔍 Customer Search": "search",
        "🔧 Service Optimization": "tools",
        "📊 Sales Insights": "bar-chart-line-fill"
    }
    sub_icons = [group_icons.get(opt, "circle") for opt in group_options]
...
if page == "📊 Executive Dashboard":
...
elif page == "🏠 Action Dashboard" and engine is not None:
    # Header
...
elif page == "👥 Customers & Leads" and df_customer is not None:
    ...
    if selected:
        ...
        with col2:
            ...
            st.markdown(f"**Loyalty Score:** {customer['loyalty_score']:.0f}")
...
elif page == "📦 Inventory & Pricing" and engine is not None:
...
# Footer
st.markdown("---")
st.caption("© 2026 Qatar Dealer AI Platform Pro | Powered by Advanced AI & Machine Learning")